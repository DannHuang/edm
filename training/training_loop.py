...
from training.sigma_model import finetune_wrapper
...
def sigma_training_loop(
    run_dir             = '.',      # Output directory.
    pretrain_dm         = '',       # Pre-trained DPM.
    dataset_kwargs      = {},       # Options for training set.
    data_loader_kwargs  = {},       # Options for torch.utils.data.DataLoader.
    network_kwargs      = {},       # Options for diffusion model and preconditioning.
    sigma_network_kwargs= {},       # Options for sigma model and preconditioning.
    loss_kwargs         = {},       # Options for loss function.
    optimizer_kwargs    = {},       # Options for optimizer.
    augment_kwargs      = None,     # Options for augmentation pipeline, None = disable.
    seed                = 0,        # Global random seed.
    ...
    lr_rampup_kimg      = 10000,    # Learning rate ramp-up duration.
    lr_anneal_kimg      = 10000,    # Learning rate anneal duration.
    loss_scaling        = 1,        # Loss scaling factor for reducing FP16 under/overflows.
    kimg_per_tick       = 50,       # Interval of progress prints.
    snapshot_ticks      = 50,       # How often to save network snapshots, None = disable.
    state_dump_ticks    = 500,      # How often to dump training state, None = disable.
    ...
    # Select batch size per GPU.
    batch_gpu_total = batch_size // dist.get_world_size()       # total batch-size per GPU
    if batch_gpu is None or batch_gpu > batch_gpu_total:
        # if batch size per GPU is unlimited or enough
        batch_gpu = batch_gpu_total
